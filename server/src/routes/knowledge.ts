...
import { syncItem as syncToSearch, deleteItem as deleteFromSearch } from '../services/searchService';
import { generateAndStoreQuestions } from '../services/reviewQuestionService';
...
export async function knowledgeRoutes(
  ...
  app.post('/api/knowledge', async (req, reply) => {
    ...
    try {
      ...
      for (let i = 0; i < result.items.length; i++) {
        try {
          syncToSearch(result.items[i].id, result.items[i].title, result.items[i].raw_content || '');
        } catch {}
      }
  ...
  app.post('/api/knowledge/upload', async (req, reply) => {
    try {
      ...
      // Sync to search index (non-blocking)
      try { syncToSearch(result.item.id, result.item.title, result.item.raw_content || ''); } catch {}
  ...
  app.put('/api/knowledge/:id', async (req, reply) => {
    ...
    // Sync to search index (non-blocking)
    try {
      syncToSearch(id, updated.title, updated.raw_content || '');
    } catch {}
  ...
  app.delete('/api/knowledge/:id', async (req, reply) => {
    ...
    // Sync deletion to search index
    try { deleteFromSearch(id); } catch {}
  ...
  // Split-preview disabled
  app.post('/api/knowledge/split-preview', async (req, reply) => {
    return reply.status(403).send({ error: 'Split preview disabled' });
  });

  // Save pieces disabled
  app.post('/api/knowledge/save-pieces', async (req, reply) => {
    return reply.status(403).send({ error: 'Save pieces disabled' });
  });
}
